...
type commission struct {
	InsuranceFee  uint `json:"insurance_fee"`
	AssistanceFee uint `json:"assistance_fee"`
	DrivyFee      uint `json:"drivy_fee"`
}
...
func main() {
	...
	for _, rental := range i.Rentals {
		...
		commissionPrice := uint(float32(price) * 0.3)
		var commissionData commission
		commissionData.InsuranceFee = commissionPrice / 2
		commissionData.AssistanceFee = days * 100
		commissionData.DrivyFee = commissionPrice - commissionData.InsuranceFee - commissionData.AssistanceFee
